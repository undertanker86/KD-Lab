...
class RAFDataModule(L.LightningDataModule):
    def __init__(
        self, train_path="./",val_path="./",test_path="./" ,batch_size=64, num_workers=0, height_width=(48, 48),
        train_transform=None, test_transform=None
    ):
    ...
    def setup(self, stage=None):
        dataset = datasets.ImageFolder(root=self.data_path)
        self.train = dataset
        self.val = datasets.ImageFolder(root=self.val_path, transform=self.test_transform)  
        self.test = datasets.ImageFolder(root=self.test_path, transform=self.test_transform)
