import sys
sys.path.append('src')
from customblock import ChannelAttention, SpatialAttention
...
class ResNet_Auxiliary(nn.Module):
    ...
    def forward(self, x, lam=0.5, index=None):
        ...
        ensemble_features = [fea[index] for fea in aux_feats]
        

        ensemle_logits = self.final_aux_classifier(ensemble_features)
 

        return aux_logits, aux_feats, ensemle_logits
...
if __name__ == '__main__':
    net = MixSKD_CIFAR_ResNet50(num_classes=100)
