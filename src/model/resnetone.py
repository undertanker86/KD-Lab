from model import Block, CBAM, SeparableConv2d
...
class ResNetOne(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=7, zero_init_residual=False,
        ...
        # self.last_conv = conv_3x3(512*self.expans, num_classes)
        #self.last_conv = nn.Linear(512*self.expans, num_classes)
        # global avg-pooling
        self.avgp = nn.AdaptiveAvgPool2d((1, 1))
        
        self.avgpool_c= nn.AdaptiveAvgPool2d((1, 1))
        self.control_v1 = nn.Linear(fix_inplanes, 4)
        self.bt_v1 = nn.BatchNorm1d(4)
...
#         print(x_c.shape)
        ...
        x_c_1=x_c[:,0].repeat(out1.size()[1], 1).transpose(0,1)
        x_c_2=x_c[:,0].repeat(out2.size()[1], 1).transpose(0,1)
        x_c_3=x_c[:,0].repeat(out3.size()[1], 1).transpose(0,1)
        x_c_4=x_c[:,0].repeat(out4.size()[1], 1).transpose(0,1)
        
        x_m = (x_c_1 * out1) + (x_c_2 * out2 ) + (x_c_3 * out3) + (x_c_4 * out4)

        return [out4, out3, out2, out1, x_m], [fea4, fea3, fea2, fea1]


def resnet18one(pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNetOne(BasicBlock, [2, 2, 2, 2], **kwargs)
    return model